  ...
  async createDocumentRequest(req, res) {
    try {
      ...
      this.sendSuccess(
        ...
        200,
        `/admin/riders/documents/${rider_id}`
      );
    } catch (error) {
  ...
  async handleRiderApprove(req, res) {
    try {
      ...
      if (!validStatuses.includes(is_approved)) {
        return res
          .status(200)
          .json({ status: 0, msg: "Invalid approval status." });
      }
      ...
      if (is_approved === "approved") {
        subject =
          "Congratulations! Your Rider Account is Approved - " +
          adminData.site_name;
      ...
      if (updatedRider) {
        ...
        return res
          .status(200)
          .json({ status: 0, msg: "Failed to approve rider." });
      }
    } catch (error) {
