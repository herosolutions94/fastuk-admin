...
const { validateEmail, validatePhoneNumber, validateRequiredFields, validateFields } = require('../../utils/validators');
...
class MemberController extends BaseController {
    ...
    async getAndInsertAddress (req, res)  {
        try {
            const { token, first_name, last_name, phone_number, address, post_code, city,memType } = req.body;
    ...
    async updateAddress (req, res)  {
        try {
            const { token, first_name, last_name, phone_number, address, post_code, city,address_id,memType } = req.body;
    ...
    async requestQuote(req, res) {
        ...
        try {
          ...
          if (token !== undefined && token !== null && token !== '' && token !== 'null') {
            ...
            if (memType==='rider') {
              return res.status(200).json({ status: 0, msg: "Rider account can not create request!" });
            }
            const userResponse = await this.validateTokenAndGetMember(token, memType);
    ...
    async paymentIntent(req, res) {
      ...
      uploadProfileImage = async (req, res) => {
        try {
          const { token,memType } = req.body; // Assuming token is sent in the request body
          if (!token) {
            return res.status(200).json({ status: 0, msg: "Token is required." });
          }
          const userResponse = await this.validateTokenAndGetMember(token, memType);
      ...
      changePassword = async (req, res) => {
        try {
          const { token, current_password, new_password, confirm_password,memType } =
            req.body;
