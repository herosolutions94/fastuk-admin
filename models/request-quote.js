...
class RequestQuoteModel extends BaseModel {
    ...
    static async getRequestQuoteByStatus(status) {
        try {
            const [rows] = await pool.query(
                `SELECT * FROM ${this.tableName} WHERE status = ?`,
                [status]
            );
            return rows; // returns all quotes with given status
        } catch (error) {
            console.error("Error fetching request quotes by status:", error);
            throw error;
        }
    }
